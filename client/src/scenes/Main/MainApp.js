              ...
              <div className='search-dropdown'>
                <ul>
                  {this.state.results.map(item => (
                    <li key={item.ItemId} unselectable="on" className={ item.ItemRarity + '-item-rarity unselectable'} onClick={() => { this.handleItemClick(item.ItemId, item.ItemName); }}>{item.ItemName}</li>
                  ))}
                </ul>
              </div>
            </div>
